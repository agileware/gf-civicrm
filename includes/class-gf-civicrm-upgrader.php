...
class Upgrader extends \Plugin_Upgrader {
    ...
    public function init() {
        ...
        // 1.12.0
        add_action( 'upgrader_process_complete', [$this, 'upgrade_version_1_12_0'], 10, 2 );
     ...
     */
    function upgrade_version_1_12_0( $upgrader, $hook_extra ) {
        // Check if we're updating this plugin
        if ( $hook_extra['action'] != 'update' || $hook_extra['type'] != 'plugin' ) {
            return;
        }
        ...
        error_log('Gravity Forms CiviCRM Integration upgrade 1.12.0 complete.');
    }
